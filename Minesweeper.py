...
class MinesweeperTile(Label):
    ...
    colormap = ['', 'blue', 'dark green', 'red', 'purple', 'maroon', 'cyan', 'black', 'dim gray']

    def __init__(self, master):
        """MinesweeperTile(master) -> Tile
    ...
    def set_inside(self, inside):
        """MinesweeperTile.set_inside(inside)
        sets what is inside the tile"""
        self.inside = inside
    ...
    def reveal(self, event):
        ...
        if not self.clicked and not self.flagged and not self.frozen:
            ...
            # change Tile to clicked and do action if Tile is empty or mined
            self.clicked = True
            self.master.sweep()
...
class MinesweeperFrame(Frame):
    ...
    def __init__(self, master, height, width, num_bombs):
        ...
        creates a new height * width minesweeper frame with num_bombs bombs"""
    ...
    def sweep(self):
        ...
        for pos in self.tiles:
            ...
            if self.tiles[pos].is_clicked() and self.tiles[pos].get_inside() == 0:
                # expose the neighboring tiles
                for np in self.get_neighbors(pos):
                    self.tiles[np].expose()

    def flag_tiles(self):
        """MinesweeperFrame.flag_tiles()
        decreases the potential mine counter if a tile has been flagged"""
        flag_count = 0
    ...
    def check_for_win(self):
        ...
        checks if all tiles without mines have been exposed"""
        is_win = True
        for pos in self.tiles:
...
def play_minesweeper(height=10, width=12, num_bombs=15):
    """play_minesweeper([height=10, width=12, num_bombs=15])
    plays minesweeper"""
    root = Tk()
    root.title('Minesweeper')
    MinesweeperFrame(root, height, width, num_bombs)
    root.mainloop()
