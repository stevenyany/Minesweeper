...
class MinesweeperTile(Button):
    ...
    def on_enter(self, event):
        '''MinesweeperTile.on_enter()
        handler for mouse on Tile
        keeps the Tile sunk'''
        # make sure Tile has been clicked, not frozen, and not a mine
        if self.clicked and not self.frozen and self.inside != '*':
            self['relief'] = SUNKEN

    def on_leave(self, event):
        '''MinesweeperTile.on_leave()
        handler for mouse away from Tile
        keeps the Tile sunk'''
        # make sure Tile ahs been clicked, not frozen, and not a mine
        if self.clicked and not self.frozen and self.inside != '*':
            self['relief'] = SUNKEN
